      ...
      {overlayToShow && (
        <div className="absolute inset-0 z-30 flex items-center justify-center transition-opacity duration-500">
          <div
            ...
            style={{
              backgroundColor: `rgba(0,0,0,${overlayToShow.dimOpacity})`,
              backdropFilter: overlayToShow.backgroundEffect.includes('blur') ? 'blur(8px)' : undefined,
            }}
          />
          <div
            className="relative z-10 flex flex-col items-center gap-4 max-w-lg"
            style={{
              alignSelf: overlayToShow.imagePosition === 'left' ? 'flex-start'
                : overlayToShow.imagePosition === 'right' ? 'flex-end' : 'center',
            }}
          >
            {overlayToShow.imageUrl && (
              <img
                src={overlayToShow.imageUrl}
                alt={overlayToShow.title || ''}
                className="rounded-lg border-2 border-primary/30 shadow-lg max-h-64 object-contain"
                style={{ transform: `scale(${overlayToShow.imageScale})` }}
              />
            )}
            {overlayToShow.title && (
              <h2 className="font-mono text-xl font-bold text-primary amber-glow text-center">{overlayToShow.title}</h2>
            )}
            {overlayToShow.subtitle && (
              <p className="font-mono text-sm text-foreground/80 text-center">{overlayToShow.subtitle}</p>
            )}
          </div>
